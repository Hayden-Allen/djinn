...
entity* script_manager::create_entity(std::string const& fp)
{
...
void script_manager::remove_entity(entity* const e)
{
	m_entity2fp.erase_key(e);
}
void script_manager::reload_file(std::string const& fp)
{
	std::string const& afp = to_absolute(fp);
	if (!m_fp2src.contains(afp))
		return;
...
void script_manager::rename_file(std::string const& old_fp, std::string const& new_fp)
{
